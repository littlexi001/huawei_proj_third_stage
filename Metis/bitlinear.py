    ...
    def svd_quant(
        ...
        if gradacc_broadcast and load_history:
            ...
            if metis_mode == "svd":
                ...
                # print(vg.requires_grad)
                # print(sg.requires_grad)
                # print(ug.requires_grad)
                # exit(1)
        ...
        if metis_mode in ["mean-hadamard", "mean_hadamard"]:
            idim = input_res.shape[-1]

            if LinearLowbitFunction.enable_nv_recipe:
                # forward Hadamard transform before quantization
                input_res = input_res @ LinearLowbitFunction.h[:idim]

            # print('input_res:', input_res.shape)
            ...
            if LinearLowbitFunction.enable_nv_recipe:
                # inverse Hadamard transform after quantization
                input_res = (
                    input_res @ LinearLowbitFunction.h[:idim].mT
                    / LinearLowbitFunction.hdim
                )
        else:
            input_res_scalar = quant_func.get_scalar(input_res)
            input_res = quant_func.quant(input_res, input_res_scalar)
