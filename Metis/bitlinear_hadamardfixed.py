...
class LinearLowbitFunction(torch.autograd.Function):
    ...
    hadamard_cache = {}   # key: (block_size, device.type, device.index, dtype)
    enable_wgrad_input_mean = False 
    enable_wgrad_only_mean_input = False # grad_weight 计算时的input要不要提mean
    
    @staticmethod
    def _check_hadamard_block_size(d: int):
        if not isinstance(d, int) or d <= 0:
            raise ValueError(f"hadamard_block_size must be a positive integer, got {d}")
        # scipy.linalg.hadamard 最自然支持 2 的幂
        if d & (d - 1) != 0:
            raise ValueError(
                f"hadamard_block_size must be a power of two, got {d}"
            )
            
    @staticmethod
    def _apply_tiled_hadamard_axis(x: torch.Tensor, axis: int, block_size: int = None) -> torch.Tensor:
        x_perm = x.movedim(axis, -1)
        y_perm = LinearLowbitFunction._apply_tiled_hadamard(x_perm, block_size)
        return y_perm.movedim(-1, axis)

    @staticmethod
    def _get_hadamard_matrix(device: torch.device, dtype: torch.dtype, block_size: int = None):
        d = block_size if block_size is not None else LinearLowbitFunction.hadamard_block_size
        LinearLowbitFunction._check_hadamard_block_size(d)

        key = (d, device.type, device.index, str(dtype))
        H = LinearLowbitFunction.hadamard_cache.get(key, None)
        if H is None:
            # 纯 Hadamard，不加随机符号对角矩阵
            H = torch.from_numpy(hadamard(d)).to(device=device, dtype=torch.float32)
            # 归一化：让它成为正交变换，前后可直接复用同一个 H
            H = H / math.sqrt(d)
            H = H.to(dtype=dtype)
            LinearLowbitFunction.hadamard_cache[key] = H
        return H

    @staticmethod
    def _apply_tiled_hadamard(x: torch.Tensor, block_size: int = None) -> torch.Tensor:
        """
        沿最后一维做 tiled Hadamard。
        例如最后一维是 4096，block_size=16，则等价于 256 个 H16。
        """
        d = block_size if block_size is not None else LinearLowbitFunction.hadamard_block_size
        LinearLowbitFunction._check_hadamard_block_size(d)
        ...
        if last_dim % d != 0:
            raise ValueError(
                f"Last dim ({last_dim}) must be divisible by hadamard_block_size ({d})."
            )

        H = LinearLowbitFunction._get_hadamard_matrix(
            device=x.device,
            dtype=x.dtype,
            block_size=d,
        )
        ...
        orig_shape = x.shape
        x2 = x.reshape(-1, last_dim)                 # [N, last_dim]
        xb = x2.view(-1, last_dim // d, d)          # [N, num_blocks, d]
        yb = torch.matmul(xb, H)                    # 每个 block 右乘 H_d
        y = yb.reshape(-1, last_dim).view(orig_shape)
        return y
    ...
    @staticmethod
    def svd_quant(
        input_:torch.Tensor, quant_func, rank=60, niter=0, metis_mode="svd", 
        ...
        original_N = 0
        if metis_mode == "svd":
            original_N = input_.shape[0]
            # 64 for Hif4
            pad_N = (64 - (original_N % 64)) % 64 
            if pad_N > 0:
                # 在第 0 维（即 Batch*SeqLen 这一维）的末尾补上 pad_N 行 0
                cinput = torch.nn.functional.pad(cinput, (0, 0, 0, pad_N))
                input_ = torch.nn.functional.pad(input_, (0, 0, 0, pad_N))
        ### add for inference ###
        ...
        if gradacc_broadcast and load_history:
            ...
            if metis_mode == "svd":
                ...
                if gradacc_broadcast:
                    ...
                    # print("reset history", history_list.get(history_id) is None)
                
                input_res = input_ - ker
                
                # print(1)
            elif metis_mode == "mean":
                me = input_.mean(dim=0, keepdim=True)
                # me = input_.mean(dim=0).repeat(1, input_.shape[0]).view(input_.shape[0], -1)
                # me *= input_.mean(dim=1, keepdim=True)
                # print(me.norm())
                input_res = input_ - me
            elif metis_mode == "mean_hadamard":
                me = input_.mean(dim=0, keepdim=True)
                input_res = input_ - me
            elif metis_mode == "meannorm":
                me = input_.mean(dim=0).repeat(1, input_.shape[0]).view(input_.shape[0], -1)
                me *= input_.mean(dim=1, keepdim=True)
                input_res = input_ - me
                # print(me.norm(),flush=True)
                # print(3)
        
        if metis_mode in ["mean-hadamard", "mean_hadamard"]:
            if LinearLowbitFunction.enable_nv_recipe:
                input_res = LinearLowbitFunction._apply_tiled_hadamard(input_res)
        ...
        if metis_mode == "svd":
            ker_scalar = quant_func.get_scalar(ker)
            ker = quant_func.quant(ker, ker_scalar)
            ker = quant_func.rquant(ker, ker_scalar)
            input_ = ker
        else:
            input_ = me 
        if broadcast_dim >= 0:
    ...
    def forward(ctx, input_: torch.Tensor, weight: torch.Tensor, bias: torch.Tensor, weight_uuid: str):     
        ...
        if LinearLowbitFunction.enable_activation_svd:
            input_ = LinearLowbitFunction.svd_quant(
                ...
                niter=LinearLowbitFunction.activation_lowrank_niter, 
                metis_mode=LinearLowbitFunction.activation_metis_mode,
                use_power_iter=LinearLowbitFunction.use_power_iter,
                key=(key, 0),              
                broadcast_dim=LinearLowbitFunction.activation_broadcast_dim,
                gradacc_broadcast=LinearLowbitFunction.gradacc_broadcast,
                load_history=load_history,
            ...
            pass
            # if LinearLowbitFunction.enable_nv_recipe:
            #     input_ = LinearLowbitFunction._apply_tiled_hadamard(input_)
        ...
        # if LinearLowbitFunction.enable_nv_recipe:
        #     weight = LinearLowbitFunction._apply_tiled_hadamard(weight)
    ...
    def backward(ctx, grad_output: torch.Tensor):
        input_, weight, bias, key = ctx.saved_tensors        
        key = int(key[0].item())
        ...
        grad_output_raw = grad_output.reshape(-1, grad_output.shape[-1]).T
        ...
        if LinearLowbitFunction.enable_backward_svd:
            if LinearLowbitFunction.backward_lowrank_svd > 0:                
                ...
                grad_output = LinearLowbitFunction.svd_quant(
                    ...
                    metis_mode=LinearLowbitFunction.gout_metis_mode,
                    use_power_iter=LinearLowbitFunction.use_power_iter,
                    key=(key, 1),              
            ...
            # =========================================================
            # 路径 A：grad_weight，不做 Hadamard
            # grad_weight = (dL/dY)^T X
            # =========================================================
            grad_output_w = grad_output_raw

            grad_output_w_scalar = LinearLowbitFunction.q_backward_outputgrad.get_scalar(grad_output_w)
            grad_output_w = LinearLowbitFunction.q_backward_outputgrad.quant(grad_output_w, grad_output_w_scalar)
            grad_output_w = LinearLowbitFunction.q_backward_outputgrad.rquant(grad_output_w, grad_output_w_scalar)

            grad_output_w_2d = grad_output_w.reshape(-1, grad_output_w.shape[-1]).T
            grad_weight = torch.matmul(grad_output_w_2d, input_.reshape(-1, input_.shape[-1]))

            # =========================================================
            # 路径 B：grad_input，只在这一路做 Hadamard
            # grad_input = q(dL/dY · H) @ q(H · W)
            # =========================================================
            grad_output_x = grad_output_raw
            weight_x = weight

            if LinearLowbitFunction.enable_nv_recipe:
                # 对 grad_output 的最后一维做 Hadamard: (..., out_features)
                grad_output_x = LinearLowbitFunction._apply_tiled_hadamard_axis(
                    grad_output_x, axis=-1
                )

                # 对 weight 的第 0 维做 Hadamard: (out_features, in_features)
                # 也就是 H @ W
                weight_x = LinearLowbitFunction._apply_tiled_hadamard_axis(
                    weight_x, axis=0
                )

            # 左因子量化：q(dL/dY · H)
            grad_output_x_scalar = LinearLowbitFunction.q_backward_outputgrad.get_scalar(grad_output_x)
            grad_output_x = LinearLowbitFunction.q_backward_outputgrad.quant(grad_output_x, grad_output_x_scalar)
            grad_output_x = LinearLowbitFunction.q_backward_outputgrad.rquant(grad_output_x, grad_output_x_scalar)

            # 右因子量化：q(H · W)
            weight_x_scalar = LinearLowbitFunction.q_backward_weight.get_scalar(weight_x)
            weight_x = LinearLowbitFunction.q_backward_weight.quant(weight_x, weight_x_scalar)
            weight_x = LinearLowbitFunction.q_backward_weight.rquant(weight_x, weight_x_scalar)

            # 注意：这里不要再做 inverse Hadamard
            # 因为我们就是要直接算 q(GH) @ q(HW)
            grad_input = torch.matmul(grad_output_x, weight_x)

            grad_output = grad_output.reshape(-1, grad_output.shape[-1]).T
            return grad_input, grad_weight, grad_bias, None
            
        if LinearLowbitFunction.enable_wgrad_only_mean_input:
            # print("gor shape", grad_output_raw.shape)
            # print("go shape", grad_output.shape)
...
class BitLinear_hadamard(nn.Module):
    def __init__(
        self, 
        in_features, 
        out_features,
        ...
        LinearLowbitFunction.activation_metis_mode = args.activation_metis_mode
        LinearLowbitFunction.gout_metis_mode = args.gout_metis_mode
        
        LinearLowbitFunction.use_power_iter = args.use_power_iter
        ...
        LinearLowbitFunction.hadamard_block_size = args.hadamard_block_size
        LinearLowbitFunction.enable_wgrad_only_mean_input = args.enable_wgrad_only_mean_input
        ...
        if self.args.enable_lowbit: 
            ...
            if self.args.enable_te:
                self.vlinear = te.Linear(
                    v.shape[1], 
                    v.shape[0], 
                    init_method=partial(BitLinear_hadamard._init_telinear, v), 
                    bias=False, 
                    device=self.device
                )
                self.ulinear = te.Linear(
                    u.shape[1], 
                    u.shape[0], 
                    init_method=partial(BitLinear_hadamard._init_telinear, u), 
                    bias=False, 
                    device=self.device
                )
            # ******************************************************************
