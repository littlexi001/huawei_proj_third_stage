...
class LinearLowbitFunction(torch.autograd.Function):
    ...
    def svd_quant(input_:torch.Tensor, quant_func, rank=60, niter=0, adaptive_schedule="none", broadcast_dim=-1):
        ...
        ug_trans = ug.T
        ug_scalar = quant_func.get_scalar(ug_trans)
        vg_scalar = quant_func.get_scalar(vg)
        ug_trans_quant = quant_func.quant(ug_trans, ug_scalar)
        ug_trans_rquant = quant_func.rquant(ug_trans_quant, ug_scalar)
        ug = ug_trans_rquant.T
...
class LinearLowbit(torch.nn.Module):
    def __init__(
        ...
        self.weight = torch.nn.Parameter(
            torch.empty((out_features, in_features), dtype=dtype, device=args.device if device is None else device)
        )
        if bias:
            self.bias = torch.nn.Parameter(
                torch.empty((out_features,), dtype=dtype, device=args.device if device is None else device)
            )
        else:
            self.bias = None
...
class BitLinear(nn.Module):
    def __init__(
        ...
        if args.enable_forward_svd == False and args.enable_lowbit == True:
            self.warmup_linear = LinearLowbit(in_features, out_features, bias=bias, args=args, dtype=dtype)
        else:
            self.warmup_linear = nn.Linear(in_features, out_features, bias=bias, device=args.device, dtype=dtype)
    ...
    def split(self):
        ...
        if self.args.enable_lowbit: 
            ...
            if self.args.forward_svd_rank > 0:
                ...
                # vlinear is low-precision 
                # ulinear, s is high-precision 
                # v must quant and rquant to use it
                self.vlinear = LinearLowbit(
                    v.shape[1], 
