...
y1 = quant_dequant_float(x_torch * 0, quant_type, force_py=True, force_fp32=True).cpu().numpy()
y2 = quant_dequant_float((x_torch * 0).cuda(), quant_type, force_py=False, force_fp32=True).cpu().numpy()
diff = np.abs(y0 - y1)
